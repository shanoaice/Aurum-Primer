...
pub async fn singbox_daemon_client_main(
    webpage_msg_reciever: AsyncReceiver<WebpageEvents>,
    tauri_app_handle: tauri::AppHandle,
    main_joinset_sender: AsyncSender<
    ...
    task_abort_handles
        .push(main_joinset.spawn(subscribe_log(log_stream, tauri_app_handle.clone())));
    task_abort_handles
        .push(main_joinset.spawn(subscribe_status(status_stream, tauri_app_handle.clone())));
